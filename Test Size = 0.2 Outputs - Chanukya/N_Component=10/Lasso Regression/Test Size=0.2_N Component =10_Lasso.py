...
# Predict continuous float values with Lasso
y_pred_continuous = regressor.predict(X_test)  

# Round Lasso outputs to nearest integer class labels and clip to valid label bounds [0, max_class]
y_pred = np.clip(np.round(y_pred_continuous), 0, len(labelencoder_y.classes_) - 1).astype(int)
...
print(classification_report(y_test, y_pred, target_names=labelencoder_y.classes_, zero_division=0))